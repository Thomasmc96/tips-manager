...
const Match = () => {
  ...
  useEffect(() => {
    axios
      .get(`${environment[0]}/server/endpoints/matches2/getById.php?id=${id}`)
      .then((response) => {
  ...
  const updateMatch = (e) => {
    ...
    axios
      .post(`${environment[0]}/server/endpoints/matches2/update.php`, {
        ...
        kickOff: new Date(match.kickoff_dtm)
          .addHours()
          .toISOString()
          .slice(0, 19)
          .replace("T", " "),
        homeTeamGoals: ["", null].includes(match.home_team_goals)
          ? null
          : Number(match.home_team_goals),
        awayTeamGoals: ["", null].includes(match.away_team_goals)
          ? null
          : Number(match.away_team_goals),
        updatedDtm: new Date()
          .addHours()
          .toISOString()
          .slice(0, 19)
  ...
  const deleteMatch = (e) => {
    ...
    if (
      window.confirm(
        "Er du sikker på, at du vil slette kampen mellen " +
      ...
      setLoading(true);
      axios
        .get(`${environment[0]}/server/endpoints/matches2/delete.php?id=${id}`)
        .then((response) => {
          if (response.data.code === 200) {
            navigate("/kampe");
          } else {
  ...
  return (
    <div className="container editMatch">
      <h2>
        Opdater <span className="yellowText">kamp #{id}</span>
      </h2>
      <form className="editMatchForm" onSubmit={updateMatch}>
        ...
        <div className="actionBtns">
          <button type="button" className="deleteBtn" onClick={deleteMatch}>
            {!loading ? (
              <>Slet kamp</>
            ) : (
              <FidgetSpinner
