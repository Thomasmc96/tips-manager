...
const PodiumPart = ({placement, number}) => {
    ...
    return (
        <section className="flex flex-col justify-center items-center my-2 sm:max-w-[30%] max-w-full mx-auto">
            <h1 className={"font-bold text-6xl sm:text-5xl mb-1 " + textColor}>{number}</h1>
            <p className="text-lg">
            {placement.totalPrize} kr. {placement.names.length > 1 ? 
            <span className="text-sm">{"(" + placement.sharedPrize + " kr. til hver)"}</span>: ""
            }</p>
            ...
            {placement.names.map((first, i) => {
                ...
                <span className="mr-1" key={i}>{first}, </span>
                );
            })}
            </p>
        </section>
